...
| `LORA_R` | `16` | LoRA rank |
| `LORA_ALPHA` | `16` | LoRA alpha (미설정 시 `LORA_R`과 동일) |
| `LORA_DROPOUT` | `0.0` | LoRA dropout (소규모·불균형 데이터에서 0.05~0.1 권장) |
...
하이퍼파라미터 bayes 탐색을 위한 [`sweep.yaml`](sweep.yaml) 설정 포함. `LEARNING_RATE` × `LORA_R` × `LORA_DROPOUT` 3차원 공간에서 `eval/macro_f1`을 최대화합니다.
