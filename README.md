...
| `AUTO_TEST_SPLIT` | `1` | train.py | `1`(기본)이면 [1/9]에서 test.jsonl 없을 시 자동 분할(80/20, seed=42). `0`이면 비활성 — test 없이 val 전체로 학습/평가 |
...
| 데이터 | Train 11,605 / Val 1,276 + Test 319 (train.py 첫 실행 시 자동 80/20 분할, `AUTO_TEST_SPLIT=0`으로 비활성 가능) |
...
```bash
git clone https://github.com/JunePark2018/finetuning-script-sweep.git
cd finetuning-script-sweep
```
...
├── split_val_test.py        # val → val/test stratified 분리 (train.py 첫 실행 시 자동 호출)
├── prompts.py               # SYSTEM_MSG/USER_PROMPT 공유 모듈 (train/evaluate/inference import)
├── sweep.yaml               # W&B Sweep 탐색 공간 정의
├── setup.sh                 # GPU 서버 초기 설정
├── requirements.txt         # 의존성
├── HYPERPARAMETERS.md       # HP 설계 근거 상세 문서
└── data/                    # 데이터셋 (첫 실행 시 HF Hub에서 자동 다운로드)
    ├── train.jsonl
    ├── val.jsonl
    ├── test.jsonl           # split_val_test.py 실행 후 생성
    ├── val_original.jsonl   # split 전 백업 (재분할 시 소스)
    ├── train/
    │   ├── 검거세미밤나방/
    │   ├── 꽃노랑총채벌레/
    │   ├── ...              # 18 해충 폴더
    │   └── 정상/
    └── val/
        └── (동일 구조 — split 후에도 물리 파일은 val/에 그대로 둠)
```
...
- `evaluation_results.json` — 6개 일반 메트릭 + 7개 서비스 지표(pest_gated_f1 등) + per-class F1

HF_TOKEN이 설정되어 있으면 자동으로 `{HF_ORG}/pest-<RUN_NAME>` 레포에 업로드됩니다 (sweep 모드는 제외 — `upload_best.py`로 winner만 업로드).
...
|---|---|---|
| LoRA r | 16 | `LORA_R=8` |
| LoRA alpha | 32 (r과 독립, TML "LoRA Without Regret" 근거) | `LORA_ALPHA=16` |
| LoRA dropout | 0.05 (ALLoRA 근거, 낮은 정규화) | `LORA_DROPOUT=0.0` |
| Batch size | 6 (effective 12) | `BATCH_SIZE=4 GRAD_ACCUM=4` |
| Learning rate | 2e-4 | `LEARNING_RATE=1e-4` |
| Epochs | 3 | `NUM_EPOCHS=5` |
| Warmup | 150 steps (~5%) | `WARMUP_STEPS=100` |
| Max steps | -1 (epoch 사용) | `MAX_STEPS=250` |
| Max grad norm | 1.0 (LoRA 불안정 방어) | - |
| Weight decay | 0.01 | - |
...
20 run × 12h = **240시간**짜리 sweep 돌리기 전에, 한 조합으로 짧게 돌려 파이프라인·HP 조합이 건강한지 5\~7분 안에 확인하는 단계. mode collapse·grad 폭주 같은 문제를 여기서 catch하면 며칠 낭비 방지.

```bash
# 첫 실행: 15~18분 (모델 다운로드 포함), 이후: 5~7분
WANDB_MODE=disabled \
MAX_STEPS=20 \
EVAL_LIMIT=20 \
RUN_NAME=smoke-test \
...
| 구간 | 체크 항목 | 정상 | 불합격 → 조치 |
|---|---|---|---|
| [6/9] | train/loss 추이 | 20 step 내 2.5 → 1.0\~2.0 완만 감소 | 5 step 내 0.1 이하 급락 → **mode collapse**, HP 조정 후 재시도 |
| [6/9] | grad_norm | 전반 < 10 유지 (max_grad_norm=1.0 clip 전 값) | 50+ 스파이크 지속 → LR 낮추거나 γ 축소 |
| [8/9] | 예측 로그 다양성 | 20건에 3종 이상 클래스 출력 | 1\~2 클래스만 반복 → **mode collapse 자동 경고** (evaluate.py가 감지) |
...
> **팁**: 20 step은 짧아서 완전 붕괴만 catch. 더 꼼꼼한 검증 원하면 `MAX_STEPS=50` (12\~13분) 또는 `MAX_STEPS=100` (25분)으로 올리세요.
...
- 고정: `LORA_DROPOUT=0.05`, `WARMUP_STEPS=150` (~5%), `weight_decay=0.01`, `NUM_EPOCHS=3`, `max_grad_norm=1.0`, scheduler/batch 등
- `LORA_DROPOUT`은 3 epoch 학습 구간에서 분산만 키운다는 ALLoRA(2410.09692) 근거로 sweep에서 제외하고 0.05 고정.
- `LORA_ALPHA` 상한 32: γ=α/r=8 조합(α=64, r=8)이 VLM mode collapse 유발하는 사례 2026-04-16 sweep에서 확인되어 64 제거.
...
- **서비스 건전성 진단 (필수)**: 단순 최상위 run을 고르지 말고 `binary_pest_recall`·`normal_specificity` 균형, `fn_pest_to_normal` 절대 카운트 함께 확인.
- **Per-class F1 슬라이스**: `eval/f1_per_class/<class_name>`을 UI에서 확인. 전체 macro가 0.85여도 특정 해충 1\~2종만 F1<0.5면 서비스 부적합. evaluate.py가 학습 직후 Discord·콘솔에 경고도 띄움.
- 자세한 체크리스트는 [HYPERPARAMETERS.md §9.0](HYPERPARAMETERS.md).
...
- **wall-clock**: 12h × 20 run ≈ **240h (≈ 10일)**. Pod 오래 유지할 계획 세우기 (실측 기준, Qwen3.5-9B + A6000)
- **Sweep 중 Pod 재시작**: 같은 `wandb agent <ID>` 명령으로 다시 띄우면 중단된 지점부터 sweep 계속
- **Discord 알림**: Sweep 중에도 매 run마다 [1/9]~[9/9] 알림 감. 조용하게 하려면 Pod의 `DISCORD_WEBHOOK_URL`을 sweep 중 unset (mode collapse 등 `@everyone` 경고는 그대로 울림)
- **Sweep run 덮어쓰기 방지**: train.py 상단이 `WANDB_RUN_ID` pop + `WANDB_RESUME=never` 강제. 2026-04-17 이전 버전에선 agent 재주입으로 같은 run ID 재사용돼 데이터 wipe되는 문제 있었음
...
| evaluate.py | ⚠️ 경고 | mode collapse 감지(`@everyone`), 해충 F1<0.5 클래스 발생 시 서비스 건전성 경고 |
