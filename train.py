...
_default_run = f"{TIMESTAMP}_r{LORA_R}_a{LORA_ALPHA}_d{LORA_DROPOUT}_lr{LEARNING_RATE}_bs{BATCH_SIZE}x{GRAD_ACCUM}_{_epoch_or_step}_w{WARMUP_STEPS}"
...
try:
    # 서비스에서 고정 프롬프트 1개만 사용 — evaluate.py / inference.py와 동일해야 함
...
try:
    ...
    # RunPod 기본 경로. 로컬/다른 호스트에서는 HF_HOME env로 오버라이드.
